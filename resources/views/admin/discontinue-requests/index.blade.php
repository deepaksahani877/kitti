...
<head>
    ...
    <title>Discontinue Requests - KITTI Admin</title>
...
<body class="bg-gray-100">
            ...
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    ...
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        <a href="{{ route('admin.discontinue-requests.index') }}" class="bg-indigo-100 text-indigo-700 px-3 py-2 rounded-md text-sm font-medium">Discontinue Requests</a>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="px-4 py-6 sm:px-0">
            ...
            <p class="mt-2 text-gray-600">Manage all discontinue requests from users</p>
        </div>
        ...
        <div class="bg-white shadow rounded-lg mb-6 p-6">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-3">
                    ...
                    <a href="{{ route('admin.discontinue-requests.index') }}" class="ml-2 bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Clear</a>
                </div>
            </form>
        </div>

        <!-- Discontinue Requests Table -->
        <div class="bg-white shadow overflow-hidden sm:rounded-md">
            <div class="px-4 py-5 sm:p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">₹{{ number_format($request->registration->plan_amount ?? 0) }}</div>
                                    <div class="text-sm text-gray-500">{{ $request->registration->duration_months ?? 0 }} months</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.discontinue-requests.show', $request) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                                    @if($request->status == 'pending')
                                        <button onclick="approveDiscontinueRequest({{ $request->id }})" class="text-green-600 hover:text-green-900 mr-3">Approve</button>
                                        <button onclick="rejectDiscontinueRequest({{ $request->id }})" class="text-red-600 hover:text-red-900">Reject</button>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">No discontinue requests found</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <!-- Pagination -->
                @if(isset($discontinueRequests) && $discontinueRequests->hasPages())
                <div class="mt-6">
                    {{ $discontinueRequests->links() }}
                </div>
                @endif
            </div>
        </div>
    ...
    <script>
        function approveDiscontinueRequest(id) {
            if (confirm('Are you sure you want to approve this discontinue request?')) {
                // Implementation for approval
                console.log('Approving discontinue request:', id);
            }
        }

        function rejectDiscontinueRequest(id) {
            if (confirm('Are you sure you want to reject this discontinue request?')) {
                // Implementation for rejection
                console.log('Rejecting discontinue request:', id);
            }
        }
    </script>
</body>
